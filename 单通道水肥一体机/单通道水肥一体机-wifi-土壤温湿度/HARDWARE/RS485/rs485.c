...
*	修改记录 :
*		版本号  日期       作者            说明
*		V1.0    2017-08-04 Armstrong.zhou 根据armfly例程序修改而成
*********************************************************************************************************
*/
...
void RS485_InitTXE(void)
{
	...
	/* 配置为推挽输出，用于切换 RS485芯片的收发状态 */
	//RS485_CTRL4 = PB5
	//USART3_RTS = PB14  (RS485)
	//RS485_CTRL5 = PC5 
...
{
	RS485_TX_EN1=1;
	//RS485_TX_1_EN();	/* 切换RS485收发芯片为发送模式 */RS485_TX_EN1
}
...
static void InitHardUart(void)
{
	...
	USART_InitStructure.USART_Mode = USART_Mode_Tx | USART_Mode_Rx;		 //发送和接受使能
...
static void ConfigUartNVIC(void)
{
	...
	/*	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);  中配置中断优先级组 */
...
{	
	...
	if (USART_GetITStatus(_pUart->uart, USART_IT_RXNE) != RESET)
	{
		...
		{
			_pUart->usRxWrite = 0;    //数据存储的地址
		}
		if (_pUart->usRxCount < _pUart->usRxBufSize)
		{
			_pUart->usRxCount++;     //数据存储的个数
		}
...
uint8_t COMGetBuf(COM_PORT_E _ucPort, u8* Buf, uint16_t len)
{
	uint8_t WriteP =0;
	uint16_t u16Cnt = 0;
	uint16_t totalCnt = 0;
	...
	if(totalCnt>len)//接收字节大于BUF size 
	{
		for(u16Cnt=0;u16Cnt<len;u16Cnt++)//只保存size个数据到BUF数组中
		{
				comGetChar(_ucPort,&Buf[WriteP++]);	
		}
	}
	else	
	{
			if(totalCnt > 0)
			{
				...
				{
					comGetChar(_ucPort,&Buf[(u16Cnt)%len]);
				}
			}	
	}
...
uint8_t COM1GetBuf(u8* Buf,u16 size)
{
	...
	if(size < totalCnt)//BUF中的数据过多,直接清除
	{				
		for(u16Cnt=0;u16Cnt<size;u16Cnt++)//只保存size个数据到BUF数组中
		{
...
uint8_t COM3GetBuf(u8* Buf,u16 size)
{
	uint8_t WriteP =0;
	uint16_t u16Cnt = 0;
	uint16_t totalCnt = 0;
...
uint8_t COM4GetBuf(u8* Buf,u16 size)
{
	uint8_t WriteP =0;
	uint16_t u16Cnt = 0;
	uint16_t totalCnt = 0;
...
uint8_t COM5GetBuf(u8* Buf,u16 size)
{
	uint8_t WriteP =0;
	uint16_t u16Cnt = 0;
	uint16_t totalCnt = 0;
...
//u8  USART1_TX_BUF[256]; 			//发送缓冲,最大USART1_MAX_SEND_LEN字节
...
//串口1，发送函数
void usart1send(u8* buf, u16 uLen)
{
	u16 u16Cnt = 0;
	if(buf == NULL || uLen == 0)
	{
...
//u8  USART2_TX_BUF[256]; 			//发送缓冲,最大USART2_MAX_SEND_LEN字节
void u2_printf(char* fmt,...)  
{  
	u16 i,j; 
	va_list ap; 
	va_start(ap,fmt);
	vsprintf((char*)g_TxBuf2,fmt,ap);
	va_end(ap);
	i=strlen((const char*)g_TxBuf2);		//此次发送数据的长度
	for(j=0;j<i;j++)							//循环发送数据
	{
	 	while(USART_GetFlagStatus(USART2,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕   
			USART_SendData(USART2,g_TxBuf2[j]); 
	} 
}

//串口2，发送函数
void usart2send(u8* buf, u16 uLen)
{
	u16 u16Cnt = 0;
	if(buf == NULL || uLen == 0)
	{
	...
	{
		while(USART_GetFlagStatus(USART2,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕   
		  USART_SendData(USART2,buf[u16Cnt]); 
	}
}
...
//	va_list ap; 
//	va_start(ap,fmt);
//	vsprintf((char*)g_TxBuf3,fmt,ap);
//	va_end(ap);
//	i=strlen((const char*)g_TxBuf3);		//此次发送数据的长度
//	for(j=0;j<i;j++)							//循环发送数据
//	{
//	 	while(USART_GetFlagStatus(USART3,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕   
//			USART_SendData(USART3,g_TxBuf3[j]); 
//	} 
//}
