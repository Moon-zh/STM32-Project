...
void RS485_InitTXE(void)
{
	...
	/* 配置 PD7为推挽输出，用于切换 RS485芯片的收发状态 */
	{
		...
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14;				 //PD7端口配置
...
void RS485_3_SendBefor(void)
{
	RS485_TX_EN3=1; /* 切换RS485收发芯片为发送模式 */
}

void RS485_3_SendOver(void)
{
	RS485_TX_EN3=0;	/* 切换RS485收发芯片为接收模式 */
}
...
static void UartVarInit(void)
{
	...
	g_tUart2.SendBefor = RS485_3_SendBefor;						/* 发送数据前的回调函数 */
	g_tUart2.SendOver = RS485_3_SendOver;						/* 发送完毕后的回调函数 */
...
//		queue_push(data);  		 
//    }
//	queue_push(USART_ReceiveData(UART5));  
	UartIRQ(&g_tUart5);
...
uint8_t COM1GetBuf(u8* Buf,u8 size)
{
	//uint8 Len = 0 ;
	uint8_t WriteP =0;
	uint16_t u16Cnt = 0;
