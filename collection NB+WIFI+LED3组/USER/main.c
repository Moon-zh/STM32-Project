	...
	for(i=0,k=0;i<sensor_num;i++)
	{
		...
		if(warup|firstarm)
		{
			if(k>2)
			{
				...
				delay_ms(1000);delay_ms(1000);k=0;
			}
			if(i==sensor_num)warup=0;
		}
	}
	if(!a[0])firstarm=1;
}
...
void	setalarm()						//显示报警设置值
{
	...
	{
		SetTextValueInt32(i+war_group+1+sensor_num-1,3,Alarm_war[i].Alarm_airtemp_up/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,4,Alarm_war[i].Alarm_airtemp_dn/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,5,Alarm_war[i].Alarm_airhumi_up/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,6,Alarm_war[i].Alarm_airhumi_dn/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,7,Alarm_war[i].Alarm_light_up);		delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,8,Alarm_war[i].Alarm_light_dn);		delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,9,Alarm_war[i].Alarm_CO2_up);			delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,10,Alarm_war[i].Alarm_CO2_dn);			delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,13,Alarm_war[i].Alarm_soiltemp_up/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,14,Alarm_war[i].Alarm_soiltemp_dn/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,11,Alarm_war[i].Alarm_soilhumi_up/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,12,Alarm_war[i].Alarm_soilhumi_dn/10);	delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,15,Alarm_war[i].Alarm_EC_up);			delay_ms(100);
		SetTextValueInt32(i+war_group+1+sensor_num-1,16,Alarm_war[i].Alarm_EC_dn);			delay_ms(100);
		SetTextValueInt32(4+sensor_num-1,9,DHCP);delay_ms(100);
		SetTextValueInt32(4+sensor_num-1,30,up_time);delay_ms(100);
		SetTextValueInt32(4+sensor_num-1,31,up_wartime);delay_ms(100);
		sprintf(buf,"%s",ssid);
		SetTextValue(4+sensor_num-1,1,(unsigned char *)buf);
		sprintf(buf,"%s",password);
		SetTextValue(4+sensor_num-1,2,(unsigned char *)buf);
		sprintf(buf,"%s",ip);
		SetTextValue(4+sensor_num-1,3,(unsigned char *)buf);
		sprintf(buf,"%s",network);
		SetTextValue(4+sensor_num-1,4,(unsigned char *)buf);
		sprintf(buf,"%s",gateway);
		SetTextValue(4+sensor_num-1,5,(unsigned char *)buf);
		sprintf(buf,"%s",dns);
		SetTextValue(4+sensor_num-1,6,(unsigned char *)buf);
	}
	delay_ms(200);
...
void	readflash()						//从flash读取报警设置
{
	...
	STMFLASH_Read(400+FLASH_SAVE_ADDR+202,(u16*)&data,1);
...
void	sendflash()						//存储报警设置到flash
{
	...
	STMFLASH_Write(602+FLASH_SAVE_ADDR,(u16*)&bz,1);
...
rest:			sendok=1;emw_set=1;Emw_B=0;
		...
		if(wifiup){wifiup=0;emw_set=1;Uptoaliyun_wifi(sensor[g],g);if(++g==sensor_num)g=0;emw_set=0;}
	}
}
