...
void	setalarm()						//显示报警设置值
{
	...
	for(i=0;i<war_group;i++)
	{
		...
		sprintf(buf,"%s",ssid);
		SetTextValue(2,20,(unsigned char *)buf);								delay_ms(100);
		sprintf(buf,"%s",password);
		SetTextValue(2,21,(unsigned char *)buf);								delay_ms(100);
	}
	delay_ms(200);
...
void	readflash()						//从flash读取报警设置
{
	...
	STMFLASH_Read(750+FLASH_SAVE_ADDR,buf,10);
	for(k=i=0;i<20;i+=2,k++)
	{
		password[i]=buf[k]>>8;
...
void	sendflash()						//存储报警设置到flash
{
	...
	STMFLASH_Write(750+FLASH_SAVE_ADDR,(u16*)buf,10);
	delay_ms(200);
	readflash();
}

void	UpSetAlarm_wifi(Waring data,u8 group);
void	UpSetAlarm_wifi2(Waring data,u8 group);
...
{	  
	...
	while(1)
	{
		...
		if(SaveButton==1)			//设置按钮按下，获取设置的值
		{
			...
			OSTaskResume(UpyunWF_TASK_PRIO);
			OSTaskSuspend(Upyun_TASK_PRIO);
			if(N21_B)UpSetAlarm_wifi2(Alarm_war[0],0);
			OSTaskResume(Upyun_TASK_PRIO);
			setalarm();							//更新触摸屏设置显示
		}
	}
...
void	Uptoaliyun_wifi2(Environmental data,u8 group)	//上传传感器数据到阿里云
{
	char a[250];
	char c[250];
	char *b=c;
...
void	UpSetAlarm_wifi2(Waring data,u8 group)			//上传报警设置值到阿里云
{
	char a[300];
	char c[300];
	char *b=c;
...
void	readset(u8 t)									//读取阿里下发的数据
{
	...
	if(len>10)
	{
		...
		UpSetAlarm_wifi2(Alarm_war[0],0);
		OSTaskResume(Alarm_TASK_PRIO);
...
void 	Upyun_task(void *pdata)							//上传云任务 N21
{
	...
	while(1)
	{
		delay_ms(200);
		readset(0);						//读取阿里云下发数据
		if(((++i)/250)>=up_time){i=0;Uptoaliyun_wifi2(sensor[0],0);}
	}
}
...
void 	UpyunWF_task(void *pdata)						//上传云任务 WIFI
{
	...
	while(1)
	{
		delay_ms(200);
		readset(1);						//读取阿里云下发数据
		if(((++i)/250)>=up_time){i=0;Uptoaliyun_wifi(sensor[0],0);}
	}
}
