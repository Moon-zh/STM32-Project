...
//更新寄存器表
u8 UpdateModbusReg(void)
{
	u8 u8Data[5] = {0,0,0,0,0};
	u8 Cnt = 0;
	//
	FlashReadWaterADD(u8Data);
	ModbusReg[Cnt++] = u8Data[0];
	ModbusReg[Cnt++] = u8Data[1];
	ModbusReg[Cnt++] = u8Data[2];
	ModbusReg[Cnt++] = u8Data[3];
	ModbusReg[Cnt++] = u8Data[4];
	FlashReadStir(u8Data);
	ModbusReg[Cnt++] = u8Data[0];
	ModbusReg[Cnt++] = u8Data[1];
	ModbusReg[Cnt++] = u8Data[2];
	ModbusReg[Cnt++] = u8Data[3];
	ModbusReg[Cnt++] = u8Data[4];
	if(Cnt>= MAX_REG_NUM)//个数超过最大个数时报错
	{
		Cnt = 0;
	}
	return Cnt;
...
//更新输入寄存器表
u8 UpdateModbusInReg(void)
{
	u8 Cnt = 0;
	ModebusInReg[Cnt++] = (g_FaultCode>>24&0x0f);
	ModebusInReg[Cnt++] = (g_FaultCode>>16&0x0f);
	ModebusInReg[Cnt++] = (g_FaultCode>>8&0x0f);
	ModebusInReg[Cnt++] = (g_FaultCode&0x0f);
	ModebusInReg[Cnt++] = LevelData[0];
	ModebusInReg[Cnt++] = LevelData[1];
	ModebusInReg[Cnt++] = LevelData[2];
	ModebusInReg[Cnt++] = LevelData[3];
	ModebusInReg[Cnt++] = LevelData[4];
	ModebusInReg[Cnt++] = StirTimeMin[0];
	ModebusInReg[Cnt++] = StirTimeMin[1];
	ModebusInReg[Cnt++] = StirTimeMin[2];
	ModebusInReg[Cnt++] = StirTimeMin[3];
	ModebusInReg[Cnt++] = StirTimeMin[4];
	ModebusInReg[Cnt++] = BucketPercent[0];
	ModebusInReg[Cnt++] = BucketPercent[1];
	ModebusInReg[Cnt++] = BucketPercent[2];
	ModebusInReg[Cnt++] = BucketPercent[3];
	ModebusInReg[Cnt++] = BucketPercent[4];
	
	if(Cnt>= MAX_INREG_NUM)//个数超过最大个数时报错
	{
		Cnt = 0;
	}
	return Cnt;
...
void SetSingleCoil(u8 *Buf)
{
	...
	if(DataAddr <= 0x04)
		...
		if(DataVal == 0xff00)
		{
			Msgtemp.CmdType = MSG_WATERADDSTART;
			Msgtemp.CmdSrc = GPRS_TASK_CODE;
			Msgtemp.CmdData[0] = DataAddr+1;
			OS_ENTER_CRITICAL();
			PackSendWaterAddQ(&Msgtemp);
			OS_EXIT_CRITICAL();
		}
		else if(DataVal == 0x0000)
		{
			Msgtemp.CmdType = MSG_WATERADDCANCEL;
			Msgtemp.CmdSrc = GPRS_TASK_CODE;
			Msgtemp.CmdData[0] = DataAddr+1;
			OS_ENTER_CRITICAL();
			PackSendWaterAddQ(&Msgtemp);
	...
	else if((DataAddr >= 0x05)&&(DataAddr <= 0x09))
	{
		if(DataVal == 0xff00)
		{
			Msgtemp.CmdType = MSG_STIRSTART;
			Msgtemp.CmdSrc = GPRS_TASK_CODE;
			Msgtemp.CmdData[0] = DataAddr-4;
			OS_ENTER_CRITICAL();
		...
		{
			Msgtemp.CmdType = MSG_STIRCANCEL;
			Msgtemp.CmdSrc = GPRS_TASK_CODE;
			Msgtemp.CmdData[0] = DataAddr-4;
			OS_ENTER_CRITICAL();
			PackSendStirQ(&Msgtemp);
...
//	}
	...
	{
		OS_ENTER_CRITICAL();
		SaveTemp[DataAddr] = DataVal;
		FlashReadWaterADD((u8*)&DataVal);
		OS_EXIT_CRITICAL();
		//return;
	}
	else if((DataAddr >= 0x05 )&&(DataAddr<=0x09))
	{
		OS_ENTER_CRITICAL();
		SaveTemp[DataAddr-5] = DataVal;
		FlashWriteStir((u8*)&DataVal);
		OS_EXIT_CRITICAL();
		//return;
	}
...
void gprs_task(void *pdata)
{
	...
	while(1)
	{
		...
		{
			//解析
			if((ModbusPara.TxRxSwitch == 0) && (COMGetBuf(COM1 ,&NETCircle, 256) > 4)&&(ModbusPara.Busy == 0))
			{
				ModbusPara.Busy = 1;
