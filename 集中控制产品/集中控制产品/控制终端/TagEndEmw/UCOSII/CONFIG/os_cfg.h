...
#define OS_MAX_EVENTS            50u   //应用中最多事件控制块的数目 必须大于0   ,每增多1个,增多8个字节(>=消息邮箱+消息队列+信号量)                                                                      
#define OS_MAX_FLAGS              5u   //应用中最多事件标志组的数目	必须大于 0	,每增多1个,增多8个字节						  
#define OS_MAX_MEM_PART           20u   //最多内存块的数目           MUST be > 0 							 
#define OS_MAX_QS                 20u   //应用中最多队列控制块的数目	MUST be > 0	,每增多一个,增多24个字节(要使用队列,最少为2)				   
