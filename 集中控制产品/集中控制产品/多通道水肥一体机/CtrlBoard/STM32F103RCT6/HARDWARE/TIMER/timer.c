...
extern u8 timeset;//先开电磁阀延迟时间
//extern u8 time_flag;//开启水泵标志
u8 water_time_second_conut=0; //浇水S计时
u8 FRET_time_second_conut=0; //施肥S计时
u8 TIM3SetFlag = 0;
//u16 water_time =0;
//u16 
//通用定时器3中断初始化
...
//	u8 sendcount=0;
		MsgStruct Msgtemp;
		OS_CPU_SR  cpu_sr;
  if(MOCHINEDATE.FertilizerIoSucess==1) //施肥阀计时
	{ 
		if(FRET_time_second_conut++>=60)
		 {
			 FRET_time_second_conut =0;
			MOCHINEDATE.FertilizerCurrenttimeing++;
		 } 
		if(MOCHINEDATE.FertilizerCurrenttimeing>=MOCHINEDATE.FertilizerCurrenttime)
		{
			MOCHINEDATE.FertilizerIoSucess=0;
			FRET_time_second_conut=0;
			MOCHINEDATE.FertilizerIoNumber&=~(1<<(MOCHINEDATE.FertilizerIoStatrNum-1));//更新要开启的施肥电磁阀
			Msgtemp.CmdType = MSG_ACTION_ERTILIZER_OFF;								//施肥电磁阀关闭
			OS_ENTER_CRITICAL();
	...
	{		
		 if(water_time_second_conut++>=60)
		 {
			 water_time_second_conut =0;
			 MOCHINEDATE.WaterTimeing++;
		 }
	 		if(MOCHINEDATE.WaterTimeing>=MOCHINEDATE.WaterTime)
			{		
				MOCHINEDATE.WaterOnSucess=0;
					water_time_second_conut=0;

					Msgtemp.CmdType = MSG_ACTION_WATER_OFF;								//浇水电磁阀关闭
		//								Msgtemp.CmdSrc = 0;
