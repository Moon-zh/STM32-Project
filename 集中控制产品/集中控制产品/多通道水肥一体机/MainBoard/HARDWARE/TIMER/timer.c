...
u8 TIM3SetFlag = 0;
extern u8 Solenoidvalve_Close_flag;//结束后需要关闭的电磁阀分区
u8 time_count=0;//时间计数
...
//	MsgStruct Msgtemp;
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
	{
		...
		LED_Statue ^= 1;
	  if(Solenoidvalve_Close_flag==1)	//有需要关闭的电磁阀
		{
			time_count++;
			if(time_count>3) //3s结束关掉电磁阀
			{
				Solenoidvalve_Close_flag=0;
				time_count=0;
