...
OS_EVENT * RemoteQMsg;
GM3Struct RemoteData;
void* 	RemoteMsgBlock[4];
OS_MEM* RemotePartitionPt;
u8 g_u8RemoteMsgMem[20][4];
//创建消息队列和内存块
//输出0：创建正常
//    1：消息队列创建失败
//	  2：内存块创建失败
u8 RemoteQInit(void)
{
	...
	RemoteQMsg = OSQCreate ( RemoteMsgBlock, 4);
...
void SetMulReg(u8* Buf)
{
	...
	for(i=0;i<DataNum;i++)
	{
		ModbusReg[DataAddr+i] = Buf[7+i*2]<<8| Buf[8+i*2];
	}
	if(DataAddr<=0x3b)
	{
		FlashWriteWaterProg((u8*)&ModbusReg[0],120);
	}
	else if((DataAddr > 0x3b)&&(DataAddr <= 0x4f))
	{
		FlashWriteFertilizerAmount((u8*)&ModbusReg[60],40);
	}
	else if((DataAddr > 0x4f)&&(DataAddr <= 0xb3))
	{
		FlashWriteDosingProg((u8*)&ModbusReg[80],200);
	}
	else if((DataAddr > 0xb3)&&(DataAddr <= 0xef))
	{
		FlashWriteIrrigationProg((u8*)&ModbusReg[180],120);
	}
	else if((DataAddr > 0xef)&&(DataAddr <= 0xf9))
	{
		FlashWriteDosingProg((u8*)&ModbusReg[240],20);
	}
}
//解析从云端收到的消息
//输入：收到的报文
...
u8 FrameRdInSta(u8* RxBuf, u8* TxBuf, u8 Flag)
{
	...
	if(Flag == 1)
	{
		TxBuf[Cnt++] = RxBuf[2];//起始地址
		TxBuf[Cnt++] = RxBuf[3];
		TxBuf[Cnt++] = MAX_INSTA_NUM>>8;//数据个数
		TxBuf[Cnt++] = MAX_INSTA_NUM%256;
		TxBuf[Cnt++] = MAX_INSTA_NUM/8;//字节数
	}
	else
	{
...
u8 FrameRdInReg(u8* RxBuf, u8* TxBuf,u8 flag )
{
	...
	u16 ReDataAddr = RxBuf[2]<<8|RxBuf[3];
	u16 ReDataNum =  RxBuf[4]<<8|RxBuf[5];
//	ModebusInReg[0] = FlowmeterCount.Flowmeter1>>16;
//	ModebusInReg[1] = FlowmeterCount.Flowmeter1&0xffff;
//	ModebusInReg[2] = FlowmeterCount.Flowmeter2>>16;
//	ModebusInReg[3] = FlowmeterCount.Flowmeter2&0xffff;
//	ModebusInReg[4] = FlowmeterCount.Flowmeter3>>16;
//	ModebusInReg[5] = FlowmeterCount.Flowmeter3&0xffff;
//	ModebusInReg[6] = FlowmeterCount.Flowmeter4>>16;
//	ModebusInReg[7] = FlowmeterCount.Flowmeter4&0xffff;
//	ModebusInReg[8] = FlowmeterCount.Flowmeter5>>16;
//	ModebusInReg[9] = FlowmeterCount.Flowmeter5&0xffff;
//	ModebusInReg[10] = FlowmeterCount.Flowmeter6;
//	ModebusInReg[11] = FlowmeterCount.Flowmeter7;
...
void gprs_task(void *pdata)
{
	...
	MsgStruct Msgtemp;	
	for(SendNum=0;SendNum<20;SendNum++)
	{
		ModebusInReg[SendNum]=SendNum;
	}
	SendNum = 0;
	delay_ms(1000);
