...
//RTC时钟中断
//每秒触发一次  
extern u16 WarterRemainderTime;
extern u16 FertilizerRemainderTime;
u8 SecCnt = 0;
u8 SecCnt2 = 0;
	...
	{							
		...
		if(WarterRemainderTime>0)
		{
			if(SecCnt++ >= 60 )
			{
				...
				if(--WarterRemainderTime == 0)
				{
					...
					Msgtemp.CmdData[0] = 0;
					InterruptPackSendMasterQ(&Msgtemp);
				}
				//alfred 主界面显示还有多长时间
				//DisCountDown(WarterRemainderTime);
			}
		}
		if(FertilizerRemainderTime>0)
		{
			if(SecCnt2++ >= 60 )
			{
				...
				//alfred 主界面显示还有多长时间
				//DisCountDown(FertilizerRemainderTime);
			}
		}
 	}
