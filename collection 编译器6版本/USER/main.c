...
void	UpSetAlarm(Waring data,u8 group)				//上传报警设置值到阿里云
{
	...
	u16 i;i=0;
	memset(a,0,sizeof a);
	sprintf(b,"\\\"airtempUP_war\\\":%d.%02d",(data.Alarm_airtemp_up/10),(data.Alarm_airtemp_up%10));		for(;*b;b++)a[i++]=*b;
	sprintf(b,"\\\"airtempDN_war\\\":%2.1f,",(data.Alarm_airtemp_dn/10.0));		for(;*b;b++)a[i++]=*b;
	sprintf(b,"\\\"airhumiUP_war\\\":%2.1f,",(data.Alarm_airhumi_up/10.0));		for(;*b;b++)a[i++]=*b;
	sprintf(b,"\\\"airhumiDN_war\\\":%2.1f,",(data.Alarm_airhumi_dn/10.0));		for(;*b;b++)a[i++]=*b;
	...
	sprintf(b,"\\\"soiltempUP_war\\\":%2.1f,",(data.Alarm_soiltemp_up/10.0));	for(;*b;b++)a[i++]=*b;
	sprintf(b,"\\\"soiltempDN_war\\\":%2.1f,",(data.Alarm_soiltemp_dn/10.0));	for(;*b;b++)a[i++]=*b;
	sprintf(b,"\\\"soilhumiUP_war\\\":%2.1f,",(data.Alarm_soilhumi_up/10.0));	for(;*b;b++)a[i++]=*b;
	sprintf(b,"\\\"soilhumiDN_war\\\":%2.1f,",(data.Alarm_soilhumi_dn/10.0));	for(;*b;b++)a[i++]=*b;
