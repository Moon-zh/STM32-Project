	...
	for(i=0;i<sensor_num;i++,k=0)
	{
		...
		if((armtime/80)>=up_wartime)
		{
			...
			if(k>2)
			{
				...
				{
					n21_set2=1;
					OSTaskSuspend(Upyun2_TASK_PRIO);
					Nsendok=sendN212(a,1);
					OSTaskResume(Upyun2_TASK_PRIO);
					n21_set2=0;
				}	
...
void 	start_task(void *pdata)							//任务开始
{
	...
	OSTaskCreate(Upyun2_task,(void *)0,(OS_STK*)&Upyun2_TASK_STK[Upyun2_STK_SIZE-1],Upyun2_TASK_PRIO);
...
void	Uptoaliyun2(Environmental data,u8 group)			//上传传感器数据到阿里云
{
	char a[250];
	char c[250];
	char *b=c;
	...
	Nsendok=sendN212(a,0);
	OSTaskResume(Alarm_TASK_PRIO);
	
}

void	UpSetAlarm(Waring data,u8 group,u8 num)			//上传报警设置值到阿里云
{
	char a[350];
	char c[350];
	char *b=c;
	...
	if(!num)Nsendok=sendN21(a,0);
	else Nsendok=sendN212(a,0);
}
...
void	readset(u8 t)									//读取阿里下发的数据
{
	...
	if(len>10)
	{
		...
		OSTaskSuspend(Alarm_TASK_PRIO);
		if(t)OSTaskSuspend(Upyun2_TASK_PRIO);
		if(N21_B2)UpSetAlarm(Alarm_war[0],0,1);
		UpSetAlarm(Alarm_war[0],0,0);
		OSTaskResume(Alarm_TASK_PRIO);
		if(t)OSTaskResume(Upyun2_TASK_PRIO);
		
	}
	else
	{
qw:		if(n21_set2)return;
		len=COM2GetBuf(buf,100);
		if(len>10)
		{
			delay_ms(200);COM2GetBuf(buf,100);
			if(strstr((const char *)buf,"RCVPUB")[0]!='R'){memset(buf,0,200);memset(c,0,200);comClearRxFifo(COM2);return;}
			msg=strstr((const char *)buf,"airtempUP_war:");	if(msg[0]=='a')Alarm_war[0].Alarm_airtemp_up=	((msg[17]-0x30)*10+(msg[18]-0x30))*10;
			msg=strstr((const char *)buf,"airtempDN_war:");	if(msg[0]=='a')Alarm_war[0].Alarm_airtemp_dn=	((msg[17]-0x30)*10+(msg[18]-0x30))*10;
			msg=strstr((const char *)buf,"airhumiUP_war:");	if(msg[0]=='a')Alarm_war[0].Alarm_airhumi_up=	((msg[17]-0x30)*10+(msg[18]-0x30))*10;
			msg=strstr((const char *)buf,"airhumiDN_war:");	if(msg[0]=='a')Alarm_war[0].Alarm_airhumi_dn=	((msg[17]-0x30)*10+(msg[18]-0x30))*10;
			...
			UpSetAlarm(Alarm_war[0],0,1);
			if(N21_B)UpSetAlarm(Alarm_war[0],0,0);
			OSTaskResume(Alarm_TASK_PRIO);
...
void	Upccid2()										//上报CCID
{
	char c[50];
	char *b=c;
	sprintf(b,"\\\"CCID\\\":\\\"%s\\\"",CCID2);
	Nsendok=sendN212(b,0);
}
...
void 	Upyun_task(void *pdata)							//上传云任务 N21
{
	...
	delay_ms(1000);
	UpSetAlarm(Alarm_war[0],0,0);delay_ms(1000);	//上报设置的报警值
	Upccid();							//上报CCID
...
void 	Upyun2_task(void *pdata)						//上传云任务 WIFI
{
	u8 g=0;
	u8 i;
	u8 buf[20];
	NeoWayN21_init2();					//N21初始化 注网
	conN212();							//N21连接到阿里云
	for(i=0;i<3;i++)					//等待N21完全初始化完
	delay_ms(1000);
	UpSetAlarm(Alarm_war[0],0,1);delay_ms(1000);	//上报设置的报警值
	Upccid2();							//上报CCID
	...
	{
		delay_ms(200);
		if(((++time1)/252)>=up_time)					//发送间隔，每次只发送一组
		{
			n21_set=1;
			comClearRxFifo(COM2);
			memset(g_RxBuf2,0,UART2_RX_BUF_SIZE);
			...
			time1=0;
			Uptoaliyun2(sensor[g],g);if(++g==sensor_num)g=0;
			n21_set=0;
		}
		if(!Nsendok){n21_set=1;N21_B=0;NeoWayN21_init2();conN212();n21_set=0;N21_B=1;}
		readset(1);						//读取阿里云下发数据	
	}
}
